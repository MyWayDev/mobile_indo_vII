...
class _DocFormState extends State<DocForm> {
  ...
  Future<List<TicketDoc>> getTicketDocs(
    ...
    if (docProblem == 'm' || docProblem == 'd') {
      response = await http
          .get('http://34.101.79.170:5000/api/missingordamageditems/$distrId');
      print('running Missing or damaged invoice: $docProblem');
    } else {
      response = await http
          .get('http://34.101.79.170:5000/api/getlateinvoices/$distrId');
  ...
  Future<List<TicketItem>> getDocItems(String docId) async {
    ...
    response = await http
        .get('http://34.101.79.170:5000/api/getinvoicedetails/$docId');
  ...
  void ticketPushToFirebase(Ticket ticket) {
    DatabaseReference ref = FirebaseDatabase.instance
        .reference()
        .child('flamelink/environments/indoProduction/content/support/en-US');
